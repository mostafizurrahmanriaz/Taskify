...
<div class="svc-form-wrap">
    <div class="svc-form-card">
        <h1 class="svc-title">Add Service</h1>
        <p class="svc-subtitle">List a new service so customers can discover and book it.</p>

        <form class="svc-form" method="POST" action="{{ route('provider.services.store') }}" enctype="multipart/form-data">
            @csrf

            <div class="svc-field svc-field--full">
                <label for="svc-title">Service Title</label>
                <input type="text" id="svc-title" name="title" placeholder="e.g. Deep House Cleaning" required>
                @error('title')
                    <span>{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="svc-row">
                ...
                <div class="svc-field">
                    ...
                    <div class="svc-price-wrap">
                        <input type="number" id="svc-price" name="price" placeholder="0.00" min="0" step="0.01" required>
                @error('price')
                    <span>{{ $message }}</span>
                @enderror
                    </div>
            ...
            <div class="svc-field svc-field--full">
                ...
                <textarea id="svc-description" name="description" rows="4" placeholder="What's included, how long it takes..."></textarea>
                @error('description')
                    <span>{{ $message }}</span>
                @enderror
            </div>
            ...
            <button type="submit" class="svc-submit">Save Service</button>
        </form>
    </div>
</div>
@endsection
